...
@Service
public class CertificadoDigitalSso {
   ...
   // @Bean
    public void ler() throws Exception {
        ...
        try {
            ...
            Connection.Response executeFinal2 = Jsoup
                    ...
                    .proxy(Proxy.usarProxy(usarProxy))
                    .followRedirects(false).method(Connection.Method.GET).execute();
            String location2 = executeFinal2.header("Location");

            Connection.Response executeFinalTribunal = Jsoup
                    .connect("https://pje1g.trf1.jus.br/pje/Processo/ConsultaProcesso/listView.seam")
                    .cookies(cookiesTribunal)
                    .ignoreContentType(true)
                    //.header("Content-type", "application/json")
    ...
    public static SSLSocketFactory socketFactoryOld() {
        try {
            final SSLContext sslContext = SSLContext.getInstance("TLS");
            final char[] password = "pjeoffice".toCharArray();
            KeyStore keyStore = KeyStore.getInstance("JKS");
            //InputStream input = CertificadoDigitalSso.class.getResourceAsStream("PJeOffice.jks");
            InputStream input = new URL("https://certificadoexpedit.s3.amazonaws.com/pjeoffice/PJeOfficeSSL.jks").openStream();
    ...
    public static SSLSocketFactory socketFactoryOld2() {
        try {
            final SSLContext sslContext = SSLContext.getInstance("TLS");
            final char[] password = "pjeoffice".toCharArray();
            KeyStore keyStore = KeyStore.getInstance("JKS");
            //InputStream input = CertificadoDigitalSso.class.getResourceAsStream("PJeOfficeSSL.jks");
            InputStream input = new URL("https://certificadoexpedit.s3.amazonaws.com/pjeoffice/PJeOffice.jks").openStream();
